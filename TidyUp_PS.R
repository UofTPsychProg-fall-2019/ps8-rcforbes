...
# reading in IAT data  ---------------------------------------------
setwd('/Users/rachelforbes/Downloads/ps8-rcforbes-master/')

# use a tidyverse function to read in the included IAT_2019.csv file 
tbl <- read.csv("IAT.csv", stringsAsFactors = FALSE) 
...
### The code from class: tbl_clean$gender <- na_if(tbl_clean$gender, '-1') would not actually change the NA values for me
### The code ran, it just did not replace NAs, I read online that na_if replaces values with NA and if_na replaces NA with values, but that function couldn't be found, so I'm not sure why it didn't work

tbl_clean$gender[is.na(tbl_clean$gender)] <-'-1'

tbl_clean$state[is.na(tbl_clean$state)] <- '-1'
...
print(gender_count[c(1,2,4),]) # three most common are 2 (men), 1(women), 5 (Genderqueer/Gender nonconforming)
...
tbl_clean$gender4 <- recode(tbl_clean$gender, '[2]' = 'men', '[1]' = 'women', '[5]'= 'Genderqueer/Gender nonconforming', .default = 'other')


# Now take a look at how highest obtained education is coded (key on line 35)
edu_count <- tbl_clean %>% group_by(edu) %>% tally()  
edu_count <- arrange(edu_count, desc(n)) # Some college
...
tbl_clean$edu7 <- recode(tbl_clean$edu, '1' = 'no highschool', '2' = 'no highschool', '3' = 'some highschool', '4' = 'highschool graduate', '5' = 'some college', '6' = 'postsecondary degree', '7' = 'postsecondary degree', '8' = 'postsecondary degree', '9' = 'masters (MA & MBA)', '14' = 'masters (MA & MBA)', '10' = 'advanced degree', '11' = 'advanced degree', '12' = 'advanced degree', '13' = 'advanced degree')
...
# white men
tbl_clean %>%
  filter(gender4=='men' & race == 6) %>% # restrict to just white men trials
  summarise(bias=mean(bias)) # compute mean

# white women
tbl_clean %>%
  filter(gender4=='women' & race == 6) %>% # restrict to just white women trials
  summarise(bias=mean(bias)) # compute mean

# advanced degree holders who are men
tbl_clean %>%
  filter(gender4=='men' & edu7 == 'advanced degree') %>% # restrict to just advanced degree men trials
  summarise(bias=mean(bias)) # compute mean

# high school graduates who are men
tbl_clean %>%
  filter(gender4=='men' & edu7 == 'highschool graduate') %>% # restrict to just high school graduates men trials
  summarise(bias=mean(bias)) # compute mean
